...
function ListUsers( props: Props ) {
		...
		render( { data } ) {
			...
			return (
				<Flex justify={"space-between"} align="center" gap={"sm"}>
					<Tooltip withArrow label={`Click to activate ${user}`}>
						<ActionIcon onClick={() => activationMutation.mutate( data?.email )} variant='outline' w="100%" color='green'>
							<IconCircleCheck />
						</ActionIcon>
					</Tooltip>
					<Tooltip withArrow label={`Click to deactivate ${user}`}>
						<ActionIcon onClick={() => deactivationMutation.mutate( data?.email )} variant='outline' w="100%" color="red">
							<IconDeselect />
						</ActionIcon>
					</Tooltip>
				</Flex>
	...
	const table = useDataGridTable( { columns, data: users ? users.reverse() : [], loading: query.isLoading, mih: "50vh" } )

	return (
		<Stack>
			<LoadingOverlay visible={query.isLoading} zIndex={999} loader={customLoader} />
